...
def _payer_radio(label: str, key: str, data: dict):
...
def _render_ai_insight_block(subsection_code: str, title: str):
    ...
    if st.button(ask_label, key=f"btn_ai_{subsection_code}"):
        # TODO: Replace this placeholder with a real GPT call + Knowledge retrieval.
        #
        # You can:
        #  - Read st.session_state[SECTION7_KEY] for current choices
        #  - Look at city/county from Section 1
        #  - Pull agent notes from your Knowledge folder
        #  - Call your tuned GPT model and store the answer below
        #
        # For now, we just store a placeholder sample text.
    ...
    # If we have an answer in session, show it with a Close button
    if subsection_code in ai_state and ai_state[subsection_code]:
        st.markdown(ai_state[subsection_code])
        if st.button(f"Close AI insights for {subsection_code}", key=f"btn_close_ai_{subsection_code}"):
            ai_state[subsection_code] = ""
...
def render_section7_allocation_costs():
    ...
    _payer_radio("Natural Hazard Disclosure (NHD) report", "nhd_payer", data)
    ...
    with col2:
        _payer_radio("Report 1 payer", "other_report_1_payer", data)
    ...
    with col2:
        _payer_radio("Report 2 payer", "other_report_2_payer", data)
    ...
    with col2:
        _payer_radio("Payer for other retrofit work", "other_gov_retrofit_payer", data)
    ...
    _payer_radio("Escrow fee", "escrow_fee_payer", data)
    ...
    _payer_radio("Owner’s title insurance policy", "owners_title_payer", data)
    ...
    _payer_radio("County transfer tax / fee", "county_transfer_tax_payer", data)
    _payer_radio("City transfer tax / fee", "city_transfer_tax_payer", data)

    st.markdown("**HOA-related fees (if applicable)**")
    _payer_radio("HOA transfer fee", "hoa_transfer_fee_payer", data)
    _payer_radio(
    ...
    _payer_radio("HOA certification fee", "hoa_cert_fee_payer", data)
    ...
    with col2:
        _payer_radio("Other cost 1 payer", "other_cost_1_payer", data)
    ...
    with col2:
        _payer_radio("Other cost 2 payer", "other_cost_2_payer", data)
